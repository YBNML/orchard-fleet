...
중심선을 만드는 건 **형태학적 세선화(thinning)** 하나다 (skeletonize).
자유공간을 위상을 보존한 채 경계에서 균등하게 깎아 들어가면 1픽셀 두께 곡선이
남고, 그게 통로 한가운데를 지난다. 통로 위치를 가정하거나 직선을 피팅하지 않는다.

거리변환은 중심선을 찾는 데 쓰지 **않는다**. 세선화가 이미 중앙을 주므로,
거리변환은 그 지점의 통로 폭을 재는 용도로만 쓴다 (RouteGraph 의 width).
다만 둘은 같은 곳을 가리킨다 — 2026-07-26 실측: 나무 구역 5,400 행 전부에서
세선화 위치와 거리변환 최댓값 위치가 1셀 이내로 일치했다.
