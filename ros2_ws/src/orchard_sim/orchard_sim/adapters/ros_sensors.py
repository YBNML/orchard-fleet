...
달라진다 — 원뿔 반각 ±8°, 자기반사 제외 r>1.6, 수관·지면 제외 z∈(−0.50, 0.75)
(MID-360), 하위 10% 백분위, 점 200개 하한, 3프레임 솎기, 밀착 반경 0.8 m 는
control_agent 에 있던 식과 주석을 그대로 옮겼다.

z 대역은 **센서 기준**이라 라이다 높이가 바뀌면 같이 옮겨야 한다. 뜻은 언제나
"지상 0.3~1.5 m 의 벽 면"이고, 2026-08-15 상부 하이브리드에서 라이다가
0.645→0.80 m 로 올라가면서 (−0.35, 0.9) → (−0.50, 0.75) 로 재도출했다
(스펙 ④ §3. 안 옮기면 대역이 통째로 15.5 cm 위로 밀려 지면·수관 판정이 어긋난다).
...
class RosSensors(Localizer, Perception):
    ...
    def feed_cloud(self, msg):
        ...
        cone = (ang < math.radians(8.0)) & (r > 1.6) \
            & (p[:, 2] > -0.50) & (p[:, 2] < 0.75)  # 수관·자기반사 제외 (MID-360)
