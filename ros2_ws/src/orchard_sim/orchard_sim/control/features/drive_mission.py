...
class DriveMission(Feature):
    ...
    def cross_y(self, sign):
        ...
        0.72(±34.3)였을 때: 종방향 칸 오차(±1.5 m)가 있으면 출구가 일찍
        끝나 램프 대역 밖(|y|≈31.8)에서 건너게 되는데, 거기는 하네스 실측
        0/4(정체 38초)다. 0.90(±35.4)이면 한 칸 일찍 끝나도 |y|≥33.9 —
        검증된 대역(33.5, 4/4) 안이다. 오차를 고치는 대신 견딘다.
        둑(|y|≈38.7)까지는 3.3 m 여유가 남고 벽 도착 판정이 받친다.
        """
        return sign * (self.col_l / 2.0 + self.HL * 0.90)
