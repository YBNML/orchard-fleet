...
# 종 위상을 채택할 최소 집중도. 처음엔 0.85(사실상 꺼짐)였다 — §3 실험의
# 편의 −0.309 m 를 수관 탓으로 봤기 때문이다. 그런데 그 대부분이 센서 전방
# 오프셋 0.275 m 였음이 드러났다(08-02): 오프셋을 보정하면 잔여 편의는
# −0.03 m 다. 통로 안 종방향의 유일한 연속 절대 기준이라 다시 켠다.
# 위상은 ±(간격/2) 안에서만 유효하므로 호출측이 보정량을 작게 물려야 한다.
LONGITUDINAL_MIN_QUALITY = 0.30
