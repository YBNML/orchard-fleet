...
def lost_clock_now(mode, sim_s, wall_s):
    ...
    * `mode="sim"` → ROS 시각(sim time). 시뮬에서 '횡단 한 번'은 시뮬 시간의
    ...
    `sim_s <= 0` 은 **`/clock` 최초 도착 전**에만 성립한다 — 그때만 벽시계로
    폴백한다. rclpy `TimeSource` 에는 노화 타임아웃이 없어서, 한 번 받은 뒤
    `/clock` 이 끊겨도 마지막 수신값을 영원히 돌려준다. 즉 **이 폴백은
    '두절 감지'가 아니다**(초판 주석이 그렇게 적었던 것은 과장이다).

    끊긴 뒤 시각이 얼어붙으면 상실 타이머도 함께 멈추는데, **이 배치에서는
    무해하다**: gz 하나가 `/clock` 과 오도메트리의 **단일 출처**라, 시계가
    멈추는 상황은 곧 오도메트리도 멈춘 상황이다. 로봇은 움직이지 않고
    추정도 흐르지 않으므로 '상실'을 셀 대상 자체가 없다. 실기(RTF=1,
    `lost_clock="wall"`)에서는 이 경로를 아예 타지 않는다.

    시계가 바뀌는 순간의 처리는 호출측(`MapLocalizer._lost_t`)이 한다: 두
    시계의 값은 비교할 수 없으므로 기준점을 다시 잡는다.
    """
...
class MapLocalizer(Node):
    ...
    def _lost_t(self):
        ...
        아니다. gz 가 `/clock` 과 오도메트리의 단일 출처라 시계가 멈춘 동안은
        추정도 흐르지 않는다. 반대로 벽시계 모드에서는 종전대로 계속 흐른다
        (실기 기준).

        `/clock` **최초 도착 전**에는 벽시계로 폴백한다(도착 후 끊겨도 rclpy 는
        마지막 값을 계속 돌려주므로 폴백으로 되돌아가지 않는다 —
        `lost_clock_now` 머리말). 그 한 번의 전환에서 두 시계의 값은 서로
        비교할 수 없다 — 그대로 두면 sim(수천) ↔ monotonic(수십만) 차이가
