...
- ⚠️ **2대 동시 전 구간 정찰은 미완**: 두 기체 모두 열 끝 부근에서 자동 정지했다. 원인은 위상 슬립이 아니라 **열 끝 얇은 구조에서 구동 감시(`scan_travel`)의 상관 축퇴 오탐** + `_slip_anchor` **스냅백**(추정을 ≈1.2~1.3 m 과거 자세로 되감음)이다(정정, `docs/findings/2026-08-15-photoreal-world.md` §9.1). 그 전까지 A 통로 0~6 · B 통로 11~15 는 무개입 주행
- ⏳ 다음: 구동 감시 열 끝 신뢰 게이트 강화 · `_slip_anchor` 스냅백 완화(`docs/findings/2026-08-15-photoreal-world.md` §9.3), 통로별 정적 횡편의(최대 0.44 m) 보정, D455 인지 기능(스펙 ⑤ — `d455_link` 프레임 준비 완료)
