...
| **9개 통로 완주** | ⏳ **미완** — 로봇이 1.5 m 주행 후 정지, 원인 미규명 |
| 종방향 절대 기준 (열 끝 앵커) | ⏳ 미착수 |
| 경로 추종 정확도 | ⏳ 미측정 |

**다음 우선순위는 완주 시험이다.** 로컬리제이션은 검증됐지만, 통로를 끝까지 돌고 선회해서 다음 통로로 넘어가는 전체 순환이 아직 증명되지 않았다. 특히 **선회 구역에서 열이 시야에서 사라질 때** 어떻게 되는지가 이 설계의 마지막 관문이다 — 원래 LIO가 죽은 바로 그 지점이기 때문이다.

종방향 절대 기준(열 끝 앵커)도 완주 시험에서 필요해진다. 통로 하나가 51 m인데 그동안 종방향은 오도메트리만으로 가기 때문이다.
