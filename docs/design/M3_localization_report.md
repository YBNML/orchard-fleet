...
## 6.9 지형의 반격 — 선회 패드, 뒤집힌 도판, 그리고 무개입 완주 (08-10~11)

08-08 최종 판정은 "추정은 닫혔고 남은 것은 지형"이었다. 그 지형 수술이
이 장이다 — 그리고 수술 중에 프로젝트 전체를 관통하던 버그가 나왔다.

### 선회 평지 패드 — 경사 위 선회의 제거

`gen_heightmap --turn-pads`(기본 켬). 부스트로피돈 파리티 그대로 횡단 쌍마다
헤드랜드에 평탄 선회장을 조성한다: 짝수 k 쌍은 북단, 홀수 k 쌍은 남단, 8개.
패드 높이 = 두 통로의 평균 → 진입(통로 k 이탈)·진출(통로 k+1 재진입)이 각각
**반 단차의 직선 y-램프**(2.0 m, 피크 ≤19%)가 되고 U-호 전체는 평지에서 돈다.
직선 등판은 하네스가 이미 12/12로 증명한 동작이다 — 설계는 "경사 위 선회만
제거하면 된다"는 한 문장이다.

오프라인 검증: 수목 구간 바이트 불변(동일 seed), 호 풋프린트 평탄 1.7~4.0 cm,
임무 횡단선 횡경사 **8~14% → 0.0~0.1%**.

### 하이트맵이 3주 내내 뒤집혀 있었다

1차 하네스가 이상했다: 5개 쌍은 3/3 즉시 통과인데 큰 단차 쌍 둘이 0/3,
중간 단차 하나가 1/3. 패드는 평탄한데 로봇이 벽에 박히듯 섰다. 참값 프로브로
정체 지점을 짚으니 지형 파일(npy)은 평지 — 그런데 시뮬 안의 로봇은 y와
무관하게 x≈1.7에서 멈추고, 남측에선 텔레포트하자마자 수십 m 를 날았다
(z=+343 사출, z=−30 관통).

정착 z-프로브(텔레포트 → 정지 자세의 z·roll·pitch)를 만들어 재니 답이 나왔다:
시뮬의 값은 **npy 를 상하 반전한 값과 정확히 일치**했다. Gazebo 는 하이트맵
이미지의 첫 행을 +y(북)에 매핑하는데, 생성기는 첫 행을 남쪽부터 써 왔다.
종전 지형은 남북 대칭(테라스는 x-프로파일, 램프 블렌드도 대칭)이라 3주간
무증상이었고, **패드가 최초의 남북 비대칭 지형이라 즉시 발각**된 것이다.
수정은 한 줄(`png[::-1]`), 수정 후 정착 프로브 10/10 지점 Δ≤1 cm · 기울기 0.

교훈: 대칭은 버그를 숨긴다. 좌표 규약은 대칭이 깨지는 최초의 지물에서만
검증된다 — 그리고 그 지물이 하필 우리가 공들여 만든 처방이었다.

연쇄 수정 둘. ① 패드가 통로 쌍을 물리로 이어 자유공간이 사다리에서
**뱀형**이 됐다 — 스켈레톤 엣지 하나가 U-호 너머 두 통로에 걸쳐 끝점 변위
기준(along_row)이 무력화. 번들 빌더를 '수목 구간 안의 연속 런' 분류
(`column_runs`)로 바꿔 9개 중심선 RMSE 58 mm 복구. ② 벽 테이블 재교정 —
그리고 교정 프로브의 관측점을 통로 바닥(28·30)에서 **임무가 실제로 앵커를
쓰는 지점**(램프 31.5 · 패드 33.5)으로 옮겼다. 통로 바닥에서 재면 진입램프
면이 겉보기 벽으로 잡힌다(5:south 산포 1.84 m — 관측 위치 의존, '상수'가
아님). 관측점을 옮기니 전 단이 울타리선(±37~39)으로 균일해지고 5:south 도
산포 0.00 으로 살아났다. **교정은 측정 지점까지 복제해야 교정이다.**

수정 후 하네스: 8쌍 × P0·P9 × 3회 = **48/48 전 통과**, 균일 ~10초, 정체 0.

### run42 — 최초의 9통로 완주 (개입 3회)

역사상 처음으로 9통로를 끝까지 갔다. 사고 전 통로 안 RMS 0.099 m —
역대 최고 수준 유지. 개입 3회는 전부 지형이 아니라 **구조물·추정**이었다:

| # | 지점 | 원인 사슬 |
|---|---|---|
| 1 | (3,4)S | 남측 출구(−33.0)+tol 0.5 조기완료 → r=1 호가 y=−33 대역 통과 → **관수 입상관**(−33.0 선, 충돌 있음)에 코 박음 → 바퀴 공회전 → 데드레커닝 폭주(est y −43) → 슬립 감지는 '횡단 방위+전방 개활' 가드에 막혀 침묵 |
| 2 | (5,6)S | est 어긋난 채 얕은 횡단(y −30.8, 패드 밖 테라스 경계) → 둑 위 정체. 슬립 자동복구(후진 재돌진)는 작동했으나 같은 둑으로 재돌진 |
| 3 | (7,8)S | 깊게 돈 호(−35.9)가 패드 한가운데 서 있던 **농가 물탱크**(11.75,−36)에 접촉 → 전방 밀착 66% 정지 (감시는 정상 작동) |

수정 3종: 입상관·밸브함을 열 끝(±31.2)으로(실제 과수원 배치이기도 하다),
물탱크를 창고 옆(패드 밖)으로, 남측 출구를 북측과 같은 −34.0 으로 통일
(구근거였던 '남측 겉보기 벽이 얕다'는 패드 조성으로 소멸 — 재교정 테이블이
증명). 남측 4쌍 임무선(−34.0) 스팟 하네스 16/16.

### run43 — 무개입 완주

![run42 타임라인](../figures/run42_timeline.png)

**9통로 · 8횡단 · 약 56분 · 개입 0회.** run42 개입 지점 세 곳을 각각 30~60초에
깨끗이 통과했다. 물리적 임무 수행 관점에서 M3 의 완주 목표가 달성됐다.

### 그러나 추정은 새 병을 보였다 — 위상 굳힘 사이클

![run43 위상 굳힘](../figures/run43_phase_cement.png)

run43 의 est 는 횡방향 완벽(|ex| 중앙 1.6 cm), 종방향은 첫 선회부터 +1.4 m
지속. 공간 프로파일이 사이클을 그대로 보여준다:

| 위치 | ey (est − 참값) |
|---|---|
| 통로 끝 6 m (벽 접근) | **−0.05 ~ −0.10** (앵커가 회복) |
| U-호 통과 중 | 0 → **+1.29** (스키드 슬립 과적산) |
| 다음 통로 전체 | **+1.40 ~ +1.63** (위상이 +1.5 로 굳힘) |

호 하나당 +1.3 m 를 과적산하고, 진입하면 벽이 등 뒤라 전방 앵커가 불가하며,
위상 보정(mod 1.5)은 +1.3 에서 가장 가까운 해인 **+1.5 로 오차를 굳힌다**.
통로 끝 앵커가 회복 → 다음 호가 재발 — 통로마다 반복. 08-03 에 나무선
앵커로 노렸다가 4연패로 봉인한 바로 그 병이다.

### 후방 벽 앵커 — 거울상 악기의 다섯 번의 죽음 (run44~48)

MID-360 은 등 뒤를 본다. 진입 직후, 방금 떠난 단의 **교정된 겉보기 벽**을
후방 원뿔(ang>172°)로 재면 위상이 굳기 전에 절대 기준을 잡을 수 있다 —
나무선처럼 새 추론이 아니라 검증된 앵커의 거울상이라는 발상이었다.
발상은 옳았다: run46 통로 1 진입에서 오차가 **+1.4 고착 대신 0.11**로
떨어졌다 — 이 병의 첫 완치 실측이다. 그러나 다섯 런에 걸쳐 활동 창을
안전하게 닫는 게이트를 끝내 찾지 못했다:

| # | 런 | 실패 모드 | 대응 |
|---|---|---|---|
| ① | run44 | U-호 중 est x 가 통로 사이 → 통로 번호 오판(이웃 단 벽차 ~2 m) → est 3 m 견인, 첫 선회 좌초 | x-위상 정렬 게이트(±0.6 m) |
| ② | run45 | 호 중 est x 가 정렬 창을 '스치는' 순간 대오차 앵커 통과 → est 1.5 m 밀림, 횡보정 0.8 상한에 고착 | 자이로 회전 게이트(wz<0.15) |
| ③ | run46 | 전방 원뿔에 수관 잡음이 걸리면 후방 폴백이 안 불려 확률적 침묵 (통로 1 완치·통로 2 재발) | 폴백 경로 추가 |
| ④ | run47 | 대각 방위(hy 0.7~0.95)에서 보정의 절반이 x 로 누설 — 350회 누적 +1.1 | y-축 투영 + 방위 ±18° |
| ⑤ | run48 | **통로 안 줄기 메아리 방** — 뒤쪽 4 m 의 줄기를 벽으로 읽고 est 가 "벽 뒤 4.4 m"에 자기일관 고착(잔차 0.09), 로봇만 전진 | **봉인** |

⑤가 구조적이다: 과수원의 후방 시야엔 1.5 m 마다 줄기가 있어 **거리만으로는
벽과 줄기를 구별할 수 없고**, est 기대 창으로 거르면 전방 앵커 설계가 경고한
닭-달걀(est 가 틀리면 창이 영원히 안/잘못 열림)이 그대로 재현된다.
`rear_anchor` 는 나무선 앵커 옆에 봉인했다(기본 꺼짐, 코드·교훈 보존).

차기 후보(미시도): **후방 클러스터 거리 산포 게이트** — 벽은 원뿔을 균일
거리로 채우고(pct50−pct10 < 0.8 m), 줄기밭은 수 m 산포를 낸다. 거리가 아니라
**형상**으로 구별하는 것이 다섯 실패의 공통 교훈이다.

현 확정 상태(run49 = run43 구성): **무개입 완주 재현**(2연속). 통로 안 청정
RMS 0.089, 그러나 위상 고착은 런마다 다르게 방황한다 — run43 은 +1칸(+1.5 m)
고정, run49 는 −6~+3칸(−9~+4.5 m)까지 다단 누적 후 끝 앵커가 회복했다.
횡(|ex| 0.018)과 방위는 무결해 **주행은 영향이 없다**(조향은 AHRS+횡 위상,
도착 판정은 끝 앵커가 맡는다). RMS<0.30 게이트는 위 차기 후보(산포 게이트)와
함께 다음 세션 과제로 남긴다.
...
| 선회 통과 | ✅ **전 8횡단 무개입** (run43, 선회 평지 패드 §6.9) |
| 슬립·정지 감지 | ✅ 3중 (구조/기복 슬립 · 밀착 · 상실 격상) — 실전 검증 |
| 열 끝 앵커 (종방향 절대) | ✅ 전방(출구) 작동 · 후방(진입)은 **봉인** — §6.9 다섯 실패 |
| 요 관측 | ✅ AHRS 주인 + 위상 미세트림 (오차 0.0° 실측) |
| **9통로 완주** | ✅ **달성** — run43 무개입 완주 (9통로 · 8횡단 · 56분) |
| RMS<0.3 게이트 | ⏳ 위상 굳힘 사이클(§6.9) — 차기: 후방 산포 게이트 |

**다음 우선순위는 위상 굳힘 사이클의 종결이다** (§6.9). 물리적 완주는 끝났고
(run43·49), 남은 것은 U-호 과적산(+1.3 m/호)이 위상 +1.5 로 굳는 것의 차단이다.
후방 벽 앵커는 완치 가능성을 실증하고(run46 통로 1: 1.4 → 0.11) 다섯 실패
끝에 봉인됐다 — 차기 악기는 **후방 클러스터 거리 산포 게이트**(형상으로 벽과
줄기밭을 구별)다. 그게 서면 통로 안 RMS 0.1 m 급이 전 구간으로 확장되고
M3 의 마지막 게이트(RMS<0.3)가 닫힌다.
...
| 46 | `climb_harness` | 정책별 횡단 통과율 직접 측정 (`--mission-pairs` = 임무 파리티 8쌍 순회) |
| 47 | `calibrate_anchor_walls` | 통로×단별 겉보기 벽 실측 (관측점 = 임무 앵커 지점) |
