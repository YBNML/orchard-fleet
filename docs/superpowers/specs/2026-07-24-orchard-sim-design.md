...
│   ├ 수목 <include> × N (플랫 구조)            ├ livox_mid70   (gpu_lidar)          │
│   │   └ 과실 <visual> × 60 (개별 Label)       ├ imu_link      (imu, 200 Hz)        │
│   ├ 지주 + 와이어                             ├ navsat        (5 Hz)               │
...
**구조 규칙 (위반 시 정답 라벨이 조용히 무너짐):**

- 한 그루 = **플랫한 최상위 `<include>` 하나**
- 그 안에 수체 `<visual>` 1개 + 과실 `<visual>` N개 (**모두 같은 `apple.dae` 참조** → VRAM 평탄)
- 과실 `<visual>` 각각이 자기 `<plugin gz-sim-label-system>`을 가진다
- **행을 부모 모델로 감싸지 않는다** — gz-sim #1579 (2022-07-07 개설, 미해결): 중첩된 라벨
  `<include>`는 panoptic `labels_map`을 0으로 만들고 서로 다른 복제본에 같은 색을 준다
