...
| D12 | MID-70 마운트 | **틸트 없음, 로봇보다 40 cm 높게, 앞면 정렬** (`0.275 0 0.645 0 0 0`) | 예전 스타트업의 실제 장비 구성 재현. 상세는 §5.4 |
| D13 | 병해 판정 임계값 | **severity 0–5 정상 / 6–10 병충해** | 기존 파이프라인 계승 + 임계값 변경. 상세는 §8.8 |
...
| # | 센서 | 타입 | 포즈 | 설정 | 역할 |
|---|---|---|---|---|---|
| 1 | `livox_mid70` | `gpu_lidar` | `0.275 0 0.645 0 0 0` (**틸트 없음**) | 113×113, ±0.6143 rad, 0.05–90 m, 10 Hz | 항법 + 인지 + SLAM 입력 **전부** |
...
**MID-70 배치 — 예전 스타트업의 실제 마운트를 재현한다 (D12).**

사용자가 실제로 쓰던 구성은 다음과 같다:
- **로봇과 같은 방향** — 틸트·요 오프셋 없음 (`rpy = 0 0 0`)
- **로봇보다 약 40 cm 높은 위치** — SCOUT MINI 전고 0.245 m + 0.40 m ≈ **z = 0.645 m**
- **로봇 앞 경계와 센서 앞 경계가 일치** — 차체 앞면 x = +0.306 m, MID-70 몸체 깊이 62.7 mm
  → 센서 중심 **x ≈ 0.275 m**
- 사용자 지시: "경계선은 정밀하게 맞출 필요 없음" → 위 값은 근사이며 xacro 파라미터로 노출한다

**틸트가 없어서 생기는 결과 (설계상 감수):**

| 항목 | 계산 | 값 |
|---|---|---|
| 지면 사각 | 하단 광선 −35.2°, z = 0.645 → `0.645 / tan(35.2°)` | **전방 0.91 m 까지 안 보임** |
| 수관 상단 도달 | 상단 광선 +35.2° → `0.645 + d·tan(35.2°)` | 3.5 m 앞에서 **3.12 m 높이**까지 |

0.6 m/s 주행 시 지면 사각 0.91 m 는 약 1.5 초 분량이다. 통로가 정적인 과수원에서는 감수 가능하다.
반대로 틸트가 없어서 **수관 상단(수고 3.2 m)이 시야에 들어오는** 이점이 있다 — 하향 25°였다면
3.5 m 앞에서 2.4 m 높이까지만 봤을 것이다.

음형 장애물(고랑·구덩이) 검출은 하향 틸트가 유리하다는 보고가 있으나(Wang et al.,
*Sensors* 2024, 24(24):7929 — 0.8 m/40°에서 지면 사각 3 m → 0.21 m, 검출률 92.7%),
**실제 장비 구성 재현이 우선**이므로 틸트 없음을 기본값으로 하고 각도는 xacro 파라미터로 남긴다.

수관 촬영은 측면 카메라(#5·#6)가 맡는다. **LiDAR 기반 수관 체적/LAI 산출은 여전히 불가**하다 —
측면을 향하는 LiDAR가 없기 때문이며, 이는 D7의 직접적 귀결이다.
...
### 8.8 기존 병해 검출 파이프라인 계승 (D13)

이 프로젝트는 사용자가 예전 스타트업에서 구축한 병해 검출 파이프라인을 이어받는다.
원본: `20250707 Plant Disease Development.pptx`.

**기존 구성 — 로직은 그대로 유지한다:**

| 단계 | 내용 |
|---|---|
| 데이터 | AI Hub 배 과수원 데이터셋 (정상 / 화상병 라벨만 존재) |
| 라벨 보강 | 세부 결함 라벨이 없어 **VLM(Gemini)에 severity 0–10 채점**을 시킴 |
| 검출 | **Fast R-CNN** 으로 식물 부위(잎·과실·줄기·가지)를 검출·크롭 |
| 분류 | **ResNet18** 로 크롭을 정상/결함 이진 분류 |
| 실적 | 학습 32,461 / 검증 8,119 / 테스트 5,079 → **정확도 93.7%** |
| 화상병 단독 | 잎·과실 × 화상병 유무 4클래스 → **정확도 99.1%, F1 99.47%** |

VLM 채점 프롬프트의 severity 정의:
`1` = 매우 경미·미용상 결함(작은 마른 반점) · `5` = 식물 일부에 영향을 주는 중간 결함 ·
`10` = 심각·광범위·치명적 손상.

**변경점 — 이진 판정 임계값 (원본 pptx 대비 변경됨):**

```
severity 0 ~ 5   →  정상   (NORMAL)
severity 6 ~ 10  →  병충해 (DEFECT)
```

원본 pptx 는 `severity 0 = NORMAL, 1–10 = DEFECT` 였다. 즉 **경계가 `>0` 에서 `≥6` 으로 이동**했고,
경미한 결함(severity 1–5)은 이제 정상으로 분류된다. severity 채점 자체는 0–10 그대로 유지하고
임계값만 바꾼다 — 임계값은 설정값으로 노출해 재조정 가능하게 한다.

**시뮬레이션에서의 귀결:**

- 절차적 생성기가 나무마다 **severity 0–10 을 정답으로 함께 출력**한다. 병반 밀도·면적률·황화 정도를
  severity 에 매핑하고, 그 매핑 함수를 문서화한다
- 정답 이진 라벨은 `severity >= 6` 로 파생시킨다. 원본 severity 도 함께 보존해 임계값을
  나중에 바꿔도 재라벨링이 필요 없게 한다
- 기존 파이프라인이 **부위 검출(Fast R-CNN) → 부위별 분류(ResNet18)** 2단이므로, 시뮬레이션
  정답도 **부위 단위**로 나와야 한다. §8.3 라벨 체계의 `20 trunk / 21 branch / 30–35 leaf_* /
  40–42 fruit_*` 가 이 구조와 맞는다
- 기존 실적은 **배(화상병)** 기준이다. 이 프로젝트의 대상은 **사과**이고, pptx 의 향후 목표에도
  사과 8종(갈색무늬병·겹무늬썩음병·그을음점무늬병·부란병·붉은별무늬병·점무늬낙엽병·탄저병·화상병)이
  들어 있다. 그중 §8.7 등급 1에 해당하는 것만 시뮬레이션 대상이다
- 93.7% / 99.1% 는 **실제 사진** 기준 수치다. 시뮬레이션 렌더링에서 나온 수치와 나란히 두지 않는다
